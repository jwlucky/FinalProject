...
# Take PubDate as the article published date
...
for filename in glob.glob(r'E:\winter 2020\step 12\pubmedsample.xml'):
    with open(filename, 'r', encoding="utf-8") as content:
        ...
        df.to_csv(r'E:\winter 2020\step 12\test output\testSample21.csv', index=False, mode = 'a', header = True)
...
data = pd.read_csv(r'E:\winter 2020\step 12\test output\testSample19.csv')
data = data[data['PMID'].notna()]
data.head(5)
...
data['countReferences'].unique()


# In[142]:


def query_count(data):
    return data.query('countReferences == 0').countReferences.count()


# In[141]:


data.query('countReferences == 0').countReferences.count()


# In[151]:


data['ReferenceList'].groupby([JournalPubDate])
